...
# whether current test backdoor program was generated by weevely
is_customrized = 0
...
class SessionFile(Session):
    ...
    def __init__(self, dbpath, custom = 0, volatile = False):
        try:
        ...
        if sessiondb and isinstance(sessiondb, dict):
            ...
            if saved_url and saved_password:
                ...
                # get is_customrized from session args
                if "is_customrized" in sessiondb.keys():
                    global is_customrized
                    is_customrized = int(sessiondb.get("is_customrized"))
...
class SessionURL(Session):
    ...
    def __init__(self, url, password, custom = 0, volatile = False):

        if not os.path.isdir(sessions_path):
            os.makedirs(sessions_path)

        global is_customrized
        is_customrized = int(custom)
        ...
        for dbpath in sessions_available:
            ...
            if sessiondb and isinstance(sessiondb, dict):
                ...
                if saved_url == url and saved_password == password:
                    # save is_customrized to session args
                    sessiondb.update({'is_customrized': custom})
